...
const CartItemForBuy2 = ({ idCart, idArticle, image, name, description, options, values, price, quantity, setTotalCart }) => {
    ...
    useEffect(() => {
        // if (isLoading) return;
        // if (wasPriceAdded) return;
        // if (hasOffer && price * quantity == finalPrice) return;
        // // console.log(finalPrice);
        // setTotalCart(price * quantity, finalPrice);
        // setWasPriceAdded(true);
        // setTotalCart((prev) => ({
        //     ...prev,
        //     [idCart]: { priceWithoutDiscount: price * quantity, priceWithDiscount: finalPrice, totalDiscount: price * quantity - finalPrice },
        // }));
        setTotalCart({
            idCart,
            priceWithoutDiscount: price * quantity,
            priceWithDiscount: finalPrice,
            totalDiscount: price * quantity - finalPrice,
        });
    }, [finalPrice, hasOffer]);
        ...
        <div className="h-40 flex items-end- gap-2 bg-white p-4 rounded-3xl">
            ...
            <div className="w-2/3 h-full flex flex-col justify-around gap-3">
                ...
                <div className="flex_items-center_gap-2">
                    <p className="text-lg font-bold">{showPrice(finalPrice)}</p>
                    {hasOffer && <p className="text-lg font-bold text-gray-400 line-through">{showPrice(price * quantity)}</p>}
                </div>
            </div>
