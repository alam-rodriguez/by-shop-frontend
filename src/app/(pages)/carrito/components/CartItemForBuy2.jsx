...
const CartItemForBuy2 = ({
    ...
    return (
        <div className="h-32 flex items-end- gap-2 bg-white p-4 rounded-3xl">
            <div className="w-2/5 h-full">
                <ImageA className="w-full h-full  object-cover rounded-2xl" src={image} />
            </div>
            <div className="w-2/3 h-full flex flex-col justify-around gap-1">
                <div className="flex flex-col gap-1">
                    <p className="text-base font-bold line-clamp-1">{name}</p>
                    <p className="font-bold text-gray-500 text-xs line-clamp-2">{description}</p>
                </div>
                {options != "" && values != "" && (
                ...
                <div className="flex_items-center_gap-2">
                    <p className="text-base font-bold">{showPriceWithCurrencyUser(finalPrice, currencyArticle, currencySelected)}</p>
                    {hasOffer && (
