...
import { getPeriodsForDelivery } from "@/app/hooks/request/periods/requestsDeliveriesPeriods";
...
const page = () => {
    ...
    const handleClickPeriod = (periodId) => router.push(`/deliveries/periodos/${periodId}`);

    if (isLoadingPeriods) return <LoadingParagraph text="Buscando Historial de mis Deliveries" />;

    return (
        <div className="m-4 flex flex-col gap-4">
            <p className="text-center font-bold text-xl">Peridos de delivery</p>
            {periodsData.map((periodsDelivery) => (
                <React.Fragment key={periodsDelivery.id}>
                    <ItemDiv
                        data={[
                            { key: "Inicio Periodo", value: periodsDelivery.start_date.split("T")[0] },
                            { key: "Fin de Periodo", value: periodsDelivery.end_date.split("T")[0] },
                            { key: "Cantidad Deliveries", value: periodsDelivery.orders_count },
                            { key: "Total Dinero Deliveries", value: periodsDelivery.delivery_total_price },
                            { key: "Moneda", value: periodsDelivery.main_currency },
                            { key: "Estado periodo", value: periodsDelivery.status },
                            periodsDelivery.payout_id && { key: "Estado Pago periodo", value: "Pagado" },
                        ].filter(Boolean)}
                        onClick={() => handleClickPeriod(periodsDelivery.id)}
                    />
                </React.Fragment>
            ))}
        </div>
