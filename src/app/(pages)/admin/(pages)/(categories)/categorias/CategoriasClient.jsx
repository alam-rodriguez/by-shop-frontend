...
const CategoriasClient = () => {
    ...
    const { data, isLoading } = useGetCategoriesByType(typeCategories);
    ...
    const handleClickCategory = (idCategory = 0) => {
        // setCategorySelected(category);
        // tipo = directas;
        router.push(`/admin/categorias/${idCategory}?tipo=${type}`);
    };
    ...
    return (
        <div className="m-4">
            <p className="text-center mb-3 font-bold text-xl">Categorias {type[0].toUpperCase() + type.slice(1)}</p>
                    ...
                    <div key={category.id} onClick={() => handleClickCategory(category.id)}>
                        <div className="bg-gray-300 p-4 rounded-t-md rounded-b-none ">
                            ...
                            <div className="flex justify-between">
                                ...
                                <p>{category.created_date.split("T")[0].split("-").reverse().join("/")}</p>
                            </div>
                        </div>
                        {category.image && (
                            <Image
            ...
            <ButtonGrayDown fn={() => handleClickCategory(0)}>Crear Categoria</ButtonGrayDown>
